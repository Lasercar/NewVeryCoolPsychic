...
class PlayState extends MusicBeatState
{
	...
	// Idk how to fix this myself (there isn't a way to create an attachedsprite/ attach a sprite to another by lua?) so....
	...
	public function startVideo(name:String, forMidSong:Bool = false, canSkip:Bool = true, loop:Bool = false, playOnLoad:Bool = true)
	{
		...
		if (foundFile)
		{
			...
			if (!forMidSong)
			{
				...
				videoCutscene.onSkip = onVideoSkip;
			}
		...
		else if(!note.noAnimation)
		{
			...
			if(char != null)
			{
				...
				if(note.isSustainNote)
				{
					...
					if(!char.isAnimationFinished()) canPlay = false;
				}
