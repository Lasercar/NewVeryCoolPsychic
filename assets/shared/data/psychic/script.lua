...
function onCreate() -- no more split second frame without the black screen (in psych 0.7.1h)
	if not playedVideo and isStoryMode and not seenCutscene then
	makeLuaSprite('blackTransition', nil, -600, -200);
...
function onEndSong()
	if not allowEnd and isStoryMode then
		...
		--This tween gets stopped/the camera reset when the song changes in Psych 0.71h and I can't be bothered to figure out how to fix it
		doTweenX('camFollowPosXTwn', getProperty('camGame.scroll'), cutsceneX + 260 -(screenWidth / 2), 6, 'quadOut'); 
